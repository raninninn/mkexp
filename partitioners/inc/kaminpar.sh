...
InvokeFromDisk() {
    ...
    if [[ "${invoke_from_disk_args[print_partitioner]}" == "1" ]]; then 
		...
		>&2 echo -e "      --c-embedding-p ${invoke_from_disk_args[p]}"
		>&2 echo -e "      --c-embedding-q ${invoke_from_disk_args[q]}"
        >&2 echo "      -E"
        >&2 echo "      -T"
    ...
    if [[ -f "$graph" ]]; then
		...
		echo -n "--c-embedding-p ${invoke_from_disk_args[p]} "
		echo -n "--c-embedding-q ${invoke_from_disk_args[q]} "
        echo -n "-E "
        echo -n "-T "
